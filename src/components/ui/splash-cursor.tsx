...
function SplashCursor({
    ...
    // Disable heavy canvas effects on small / touch-first devices to preserve mobile performance
    useEffect(() => {
        try {
            const isSmall = typeof window !== 'undefined' && (window.innerWidth < 768 || window.matchMedia && window.matchMedia('(pointer: coarse)').matches);
            if (isSmall) setDisabledOnMobile(true);
        } catch (e) {
            /* ignore */
        }
    }, []);
    ...
    useEffect(() => {
        ...
        function getSupportedFormat(gl: WebGL2RenderingContext, internalFormat: number, format: number, type: any): any {
        ...
        function supportRenderTextureFormat(gl: WebGL2RenderingContext, internalFormat: number, format: number, type: any) {
